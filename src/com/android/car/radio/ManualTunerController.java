...
public class ManualTunerController {
    ...
    private void updateDisplay() {
        synchronized (mLock) {
            // Bands without manual entry (e.g. DAB) have no keypad-enterable channel; disable
            // the keypad/enter instead of calling getValidAppendices/parseDigits (which throw).
            if (!mProgramType.supportsManualEntry()) {
                mChannelDisplay.setText("");
                mEnterButton.setEnabled(false);
                for (int i = 0; i < 10; i++) {
                    mDigitButtons.get(i).setEnabled(false);
                }
                return;
            }
