...
public final class RegionConfig implements Parcelable {
    ...
    // DAB has no BandDescriptor in the framework; its support is signalled separately
    // (the module exposes a DAB frequency table). Tracked here so the DAB band shows up
    // in the selector and survives parceling.
    private final boolean mDabSupported;

    public RegionConfig(@Nullable List<BandDescriptor> amFmConfig) {
        this(amFmConfig, /* dabSupported= */ false);
    }

    public RegionConfig(@Nullable List<BandDescriptor> amFmConfig, boolean dabSupported) {
        mAmConfig = new ArrayList<>();
        mFmConfig = new ArrayList<>();
        mDabSupported = dabSupported;
        if (amFmConfig == null) {
    ...
    private List<ProgramType> createSupportedProgramTypes() {
        ...
        if (mDabSupported) supported.add(ProgramType.DAB);
        return supported;
    }
    ...
    public String toString() {
        return "RegionConfig{AM=" + mAmConfig + ", FM=" + mFmConfig + ", DAB=" + mDabSupported + "}";
    }

    @Override
    public int hashCode() {
        return Objects.hash(mAmConfig, mFmConfig, mDabSupported);
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (mDabSupported != other.mDabSupported) return false;
        return true;
    }
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeBoolean(mDabSupported);
    }
