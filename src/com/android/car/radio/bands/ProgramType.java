...
public abstract class ProgramType implements Parcelable {
    ...
    public static @Nullable ProgramType fromSelector(@Nullable ProgramSelector sel) {
        ...
        // The HAL's 64-bit DAB SId surfaces as IDENTIFIER_TYPE_DAB_DMB_SID_EXT (14);
        // accept the deprecated 32-bit type too for compatibility.
        if (priType == ProgramSelector.IDENTIFIER_TYPE_DAB_SID_EXT
                || priType == ProgramSelector.IDENTIFIER_TYPE_DAB_DMB_SID_EXT) {
            return DAB;
        }
    ...
    /**
     * Whether this band supports manual channel entry via the numeric keypad.
     *
     * <p>DAB has no manually-enterable frequency/channel in this UI (services are picked from
     * the browse list), so it returns {@code false}; the manual tuner disables its keypad and
     * never calls {@link #parseDigits}/{@link #getValidAppendices} for such bands.
     *
     * @return {@code true} if manual entry is supported (default).
     */
