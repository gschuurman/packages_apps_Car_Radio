...
public final class RadioManagerExt {
    ...
    private void initModulesLocked() {
        ...
        // The module advertises DAB by returning a non-null DAB frequency table (derived from
        // the HAL's getDabRegionConfig). Used to add the DAB band to the selector.
        mDabSupported = moduleProperties.getDabFrequencyTable() != null;
        Log.d(TAG, "DAB supported: " + mDabSupported);
    }

    /**
     * Whether the active module supports DAB (i.e. exposes a DAB frequency table).
     *
     * @return {@code true} if DAB is supported
     */
    public boolean isDabSupported() {
        synchronized (mLock) {
            initModulesLocked();
            return mDabSupported;
        }
    }
    ...
    public Bitmap getMetadataImage(long globalId) {
        return resolveMetadataImage(globalId);
    }

    /**
     * Resolves a metadata image by its global id against the open tuner sessions.
     *
     * <p>Static so any in-process component (e.g. the UI controller) can fetch artwork the
     * service already advertised, without holding a {@link RadioManagerExt} instance — the
     * tuner sessions live in the shared static {@link #sSessions}. This is a binder call to
     * the broadcast-radio service, so it must not run on the main thread.
     *
     * @param globalId Global id of the metadata image (0 is invalid)
     * @return the decoded image, or {@code null} if unavailable
     */
    @Nullable
    public static Bitmap resolveMetadataImage(long globalId) {
        if (globalId == 0) {
            return null;
        }
