...
public class RadioStorage {
    ...
    private final LiveData<List<Program>> mStations;
    ...
    private RadioStorage(Context context) {
        ...
        mStations = mDatabase.getAllStations();
    }
    ...
    /**
     * Returns the persisted Browse station list (last-seen program list). Refreshed via the live
     * object whenever {@link #saveStations} completes.
     */
    @NonNull
    public LiveData<List<Program>> getStations() {
        return mStations;
    }

    private class SaveStationsTask extends AsyncTask<List<Program>, Void, Void> {
        @Override
        protected Void doInBackground(List<Program>... programs) {
            mDatabase.saveStations(programs[0]);
            return null;
        }
    }
     ...
     *
     * @param stations Stations to persist, in display order.
     */
